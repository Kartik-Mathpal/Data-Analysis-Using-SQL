...
-- Exploring the data, Checking the shape and checking for null values.
...
select count(*) from director_mapping as total_rows;
select count(*) from genre as total_rows;
select count(*) from movie as total_rows; 
select count(*) from names as total_rows;
select count(*) from ratings as total_rows;
select count(*) from role_mapping as total_rows;
...
SELECT genre, count(id) as num_of_movies
from genre as g
...
(
SELECT movie_id, count(genre) as movies_with_one_genre
FROM genre
GROUP BY movie_id 
HAVING movies_with_one_genre = 1
) AS A;
...
INNER JOIN ratings r
ON m.id = r.movie_id
WHERE year = 2017 AND MONTH(date_published) = 3 AND m.country LIKE'%USA%' AND r.total_votes>1000
GROUP BY genre
...
SELECT n.name AS actor_name, any_value(r.total_votes) AS total_votes,
...
SELECT n.name as actress_name, count(r.total_votes), 
